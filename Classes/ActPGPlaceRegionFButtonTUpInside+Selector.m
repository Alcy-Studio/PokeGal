//
//  ActPGPlaceRegionFButtonTUpInside+Selector.m
//  frdfrd
//
...
#import "ActPGPlaceRegionFButtonTUpInside+Selector.h"
#import "Event.h"
...
@implementation ActPGPlaceRegionFButtonTUpInside(Selector)
...
-(void)setupEventForSchedule{
	...
	event.desc = kRegionDescF;
	event.name = kRegionNameF;
	event.region = kRegionF;
	
	//Edit role, color, image
	event.role = [NSNumber numberWithInt:kAudUDNumPGPlaceRegionF];
	event.color = kRegionColorF;
	event.image = [MLoad getContentWithAttr1:[NSNumber numberWithInt:kModelGeneralScenePGSchedule] 
									   ...
									   Attr3:[NSNumber numberWithInt:kModelG2DKindUiScheRegionF] 
...
-(void)switchViewToPGMainWithTFlipFromL{
	if (![MUi switchViewWithController:@"PGMainSVController" transition:UIViewAnimationTransitionFlipFromLeft])
		NSLog(@"ActPGPlaceRegionFButtonTUpInside - switchViewWithController: failed");	
}
