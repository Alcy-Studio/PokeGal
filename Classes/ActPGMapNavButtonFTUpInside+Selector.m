//
//  ActPGMapNavButtonFTUpInside+Selector.m
//  PocketDraft
//
...
#import "ActPGMapNavButtonFTUpInside+Selector.h"
...
@interface ActPGMapNavButtonFTUpInside()
...
@implementation ActPGMapNavButtonFTUpInside(Selector)
...
-(void)modifyDirectRole{
	...
	[MUi modifyTag:kRegionTagPic role:kAudUDNumPGMapRegionPicF scene:kSceneCodePGMap];
	[MUi modifyTag:kRegionTagTopLvReq role:kAudUDNumPGMapTopLvReqRegionF scene:kSceneCodePGMap];
	[MUi modifyTag:kRegionTagMidLvReq role:kAudUDNumPGMapMidLvReqRegionF scene:kSceneCodePGMap];
	[MUi modifyTag:kRegionTagLowLvReq role:kAudUDNumPGMapLowLvReqRegionF scene:kSceneCodePGMap];
	[MUi modifyTag:kRegionTagDetail role:kAudUDNumPGMapRegionFDetail scene:kSceneCodePGMap];
	
	[MUi modifyTag:kRegionTagGo role:kAudUDNumPGMapGoButtonF scene:kSceneCodePGMap];
...
-(void)modifyInDirectRole{
	...
	NSArray* array2 = [MLoad getRecordsWithAttrName:kGlossaryName attrValue:kRegionNameF Entity:kGlossaryRegion];
	if ([array2 count] != 1) {
		NSLog(@"[ActPGMapNavButton modifyInDirectRole] - Failed");
		return;
	}
...
#pragma mark Test
	...
	if(![MEvent checkEventExist:[NSString stringWithFormat:@"%@ %@",s1,s2] Region:kRegionF]){
		//set TmpAction Go
		[MUi modifyUserDefaultWithKey:kTmpAction value:kAtRegionF];
		//[self playSoundNormal];
		return;
	}
	...
	[MUi modifyUserDefaultWithKey:kTmpAction value:kAtRegionDateF];
	
	//set Role for Dating
	[MUi modifyTag:kRegionTagGo role:kAudUDNumPGMapDateButtonF scene:kSceneCodePGMap];
