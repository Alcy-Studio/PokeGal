//
//  ActPGAchiMRButtonTUpInside+Selector.m
//  PocketDraft
//
...
#import "ActPGAchiMRButtonTUpInside+Selector.h"
...
@implementation ActPGAchiMRButtonTUpInside(Selector)
...
-(void)setHintWithGiftDescriptionNone{
	NSString* s = [MLoad getContentWithAttr1:[NSNumber numberWithInt:kModelGeneralScenePGAchi] 
									   ...
									   Attr3:[NSNumber numberWithInt:kModelTextOutKindGiftHint6] 
									  Entity:kGlossaryTextOut
										 Key:kGlossaryContent];
	if (!s){
		NSLog(@"ActPGAchiMRButtonTUpInside - setHintWithGiftDescriptionNone");	
		return;
	}
	
	if (![self.resource isMemberOfClass:[PGAchiViewController class]]){
		NSLog(@"ActPGAchiMRButtonTUpInside: c is not PGAchiViewController");
		return;
	}
	id v = [[(PGAchiViewController*)self.resource view] viewWithTag:tag1];
	
	if (![v isMemberOfClass:[UILabel class]]){
		NSLog(@"ActPGAchiMRButtonTUpInside: v is not UILabel");
		return;
	}
...
-(void)setMsgWithGiftDescriptionOn{
	NSString* s = [MLoad getContentWithAttr1:[NSNumber numberWithInt:kModelGeneralScenePGAchi] 
									   Attr2:[NSNumber numberWithInt:kModelTextOutInfoGift] 
									   Attr3:[NSNumber numberWithInt:kModelTextOutKindGiftNote6] 
									  Entity:kGlossaryTextOut
										 Key:kGlossaryContent];
	if (!s){
