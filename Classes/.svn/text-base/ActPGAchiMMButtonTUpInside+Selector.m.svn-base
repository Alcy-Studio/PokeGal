//
//  ActPGAchiMMButtonTUpInside+Selector.m
//  PocketDraft
//
...
#import "ActPGAchiMMButtonTUpInside+Selector.h"
...
@implementation ActPGAchiMMButtonTUpInside(Selector)
...
-(void)setHintWithGiftDescriptionNone{
	NSString* s = [MLoad getContentWithAttr1:[NSNumber numberWithInt:kModelGeneralScenePGAchi] 
									   ...
									   Attr3:[NSNumber numberWithInt:kModelTextOutKindGiftHint5] 
									  Entity:kGlossaryTextOut
										 Key:kGlossaryContent];
	if (!s){
		NSLog(@"ActPGAchiMMButtonTUpInside - setHintWithGiftDescriptionNone");	
		return;
	}
	
	if (![self.resource isMemberOfClass:[PGAchiViewController class]]){
		NSLog(@"ActPGAchiMMButtonTUpInside: c is not PGAchiViewController");
		return;
	}
	id v = [[(PGAchiViewController*)self.resource view] viewWithTag:tag1];
	
	if (![v isMemberOfClass:[UILabel class]]){
		NSLog(@"ActPGAchiMMButtonTUpInside: v is not UILabel");
		return;
	}
...
-(void)setMsgWithGiftDescriptionOn{
	NSString* s = [MLoad getContentWithAttr1:[NSNumber numberWithInt:kModelGeneralScenePGAchi] 
									   Attr2:[NSNumber numberWithInt:kModelTextOutInfoGift] 
									   Attr3:[NSNumber numberWithInt:kModelTextOutKindGiftNote5] 
									  Entity:kGlossaryTextOut
										 Key:kGlossaryContent];
	if (!s){
