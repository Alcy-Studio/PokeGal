//
//  ActPGPlaceRegionHButtonTUpInside+Selector.m
//  frdfrd
//
...
#import "ActPGPlaceRegionHButtonTUpInside+Selector.h"
#import "Event.h"
...
@implementation ActPGPlaceRegionHButtonTUpInside(Selector)
...
-(void)setupEventForSchedule{
	...
	if (!s2 || !s3) {
		NSLog(@"setupStartDate or setupEndDate Failed");
		return;
	}
	//checkEventExist
	...
	event.desc = kRegionDescH;
	event.name = kRegionNameH;
	event.region = kRegionH;
	
	//Edit role, color, image
	event.role = [NSNumber numberWithInt:kAudUDNumPGPlaceRegionH];
	event.color = kRegionColorH;
	event.image = [MLoad getContentWithAttr1:[NSNumber numberWithInt:kModelGeneralScenePGSchedule] 
									   ...
									   Attr3:[NSNumber numberWithInt:kModelG2DKindUiScheRegionH] 
...
-(void)switchViewToPGMainWithTFlipFromL{
	if (![MUi switchViewWithController:@"PGMainSVController" transition:UIViewAnimationTransitionFlipFromLeft])
		NSLog(@"ActPGPlaceRegionHButtonTUpInside - switchViewWithController: failed");	
}
