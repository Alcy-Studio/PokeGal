//
//  ActPGAchiBRButtonTUpInside+Selector.m
//  PocketDraft
//
...
#import "ActPGAchiBRButtonTUpInside+Selector.h"
...
@implementation ActPGAchiBRButtonTUpInside(Selector)
...
-(void)setHintWithGiftDescriptionNone{
	NSString* s = [MLoad getContentWithAttr1:[NSNumber numberWithInt:kModelGeneralScenePGAchi] 
									   ...
									   Attr3:[NSNumber numberWithInt:kModelTextOutKindGiftHint9] 
									  Entity:kGlossaryTextOut
										 Key:kGlossaryContent];
	if (!s){
		NSLog(@"ActPGAchiBRButtonTUpInside - setHintWithGiftDescriptionNone");	
		return;
	}
	
	if (![self.resource isMemberOfClass:[PGAchiViewController class]]){
		NSLog(@"ActPGAchiBRButtonTUpInside: c is not PGAchiViewController");
		return;
	}
	id v = [[(PGAchiViewController*)self.resource view] viewWithTag:tag1];
	
	if (![v isMemberOfClass:[UILabel class]]){
		NSLog(@"ActPGAchiBRButtonTUpInside: v is not UILabel");
		return;
	}
...
-(void)setMsgWithGiftDescriptionOn{
	NSString* s = [MLoad getContentWithAttr1:[NSNumber numberWithInt:kModelGeneralScenePGAchi] 
									   Attr2:[NSNumber numberWithInt:kModelTextOutInfoGift] 
									   Attr3:[NSNumber numberWithInt:kModelTextOutKindGiftNote9] 
									  Entity:kGlossaryTextOut
										 Key:kGlossaryContent];
	if (!s){
