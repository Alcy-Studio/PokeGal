//
//  ActPGPlaceRegionDButtonTUpInside+Selector.m
//  frdfrd
//
...
#import "ActPGPlaceRegionDButtonTUpInside+Selector.h"
#import "Event.h"
...
@implementation ActPGPlaceRegionDButtonTUpInside(Selector)
...
-(void)setupEventForSchedule{
	...
	event.desc = kRegionDescD;
	event.name = kRegionNameD;
	event.region = kRegionD;
	
	//Edit role, color, image
	event.role = [NSNumber numberWithInt:kAudUDNumPGPlaceRegionD];
	event.color = kRegionColorD;
	event.image = [MLoad getContentWithAttr1:[NSNumber numberWithInt:kModelGeneralScenePGSchedule] 
									   ...
									   Attr3:[NSNumber numberWithInt:kModelG2DKindUiScheRegionD] 
...
-(void)switchViewToPGMainWithTFlipFromL{
	if (![MUi switchViewWithController:@"PGMainSVController" transition:UIViewAnimationTransitionFlipFromLeft])
		NSLog(@"ActPGPlaceRegionDButtonTUpInside - switchViewWithController: failed");	
}
