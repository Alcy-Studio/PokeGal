//
//  ActPGPlaceRegionEButtonTUpInside+Selector.m
//  frdfrd
//
...
#import "ActPGPlaceRegionEButtonTUpInside+Selector.h"
#import "Event.h"
...
@implementation ActPGPlaceRegionEButtonTUpInside(Selector)
...
-(void)setupEventForSchedule{
	...
	event.desc = kRegionDescE;
	event.name = kRegionNameE;
	event.region = kRegionE;
	
	//Edit role, color, image
	event.role = [NSNumber numberWithInt:kAudUDNumPGPlaceRegionE];
	event.color = kRegionColorE;
	event.image = [MLoad getContentWithAttr1:[NSNumber numberWithInt:kModelGeneralScenePGSchedule] 
									   ...
									   Attr3:[NSNumber numberWithInt:kModelG2DKindUiScheRegionE] 
...
-(void)switchViewToPGMainWithTFlipFromL{
	if (![MUi switchViewWithController:@"PGMainSVController" transition:UIViewAnimationTransitionFlipFromLeft])
		NSLog(@"ActPGPlaceRegionEButtonTUpInside - switchViewWithController: failed");	
}
