...
#import "PGScheduleViewController+IB.h"
#import "Think.h"
#import "Think+Act.h"
...
@implementation PGScheduleViewController(IB)
...
- (IBAction) touchUpInside:(id)sender{
	 ...
	 */
	Think *think = [[Think alloc] initWithScene:kSceneCodePGSchedule tag:[sender tag] event: kEventCodeTouchUpInside];
	if (![think act])
...
- (IBAction) PGTouchUpInside:(id)sender{
	...
	if ([sender tag] == kScheduleViewTitleButton) {
		action = [[ActPGScheduleTitleTUpInside alloc] initWithRole:kScheduleViewTitleButton];
	} else {
		return;
	}
