//
//  ActPGAchiURButtonTUpInside+Selector.m
//  PocketDraft
//
...
#import "ActPGAchiURButtonTUpInside+Selector.h"
...
@implementation ActPGAchiURButtonTUpInside(Selector)
...
-(void)setHintWithGiftDescriptionNone{
	NSString* s = [MLoad getContentWithAttr1:[NSNumber numberWithInt:kModelGeneralScenePGAchi] 
									   ...
									   Attr3:[NSNumber numberWithInt:kModelTextOutKindGiftHint3] 
									  Entity:kGlossaryTextOut
										 Key:kGlossaryContent];
	if (!s){
		NSLog(@"ActPGAchiURButtonTUpInside - setHintWithGiftDescriptionNone");	
		return;
	}
	
	if (![self.resource isMemberOfClass:[PGAchiViewController class]]){
		NSLog(@"ActPGAchiURButtonTUpInside: c is not PGAchiViewController");
		return;
	}
	id v = [[(PGAchiViewController*)self.resource view] viewWithTag:tag1];
	
	if (![v isMemberOfClass:[UILabel class]]){
		NSLog(@"ActPGAchiURButtonTUpInside: v is not UILabel");
		return;
	}
...
-(void)setMsgWithGiftDescriptionOn{
	NSString* s = [MLoad getContentWithAttr1:[NSNumber numberWithInt:kModelGeneralScenePGAchi] 
									   Attr2:[NSNumber numberWithInt:kModelTextOutInfoGift] 
									   Attr3:[NSNumber numberWithInt:kModelTextOutKindGiftNote3] 
									  Entity:kGlossaryTextOut
										 Key:kGlossaryContent];
	if (!s){
