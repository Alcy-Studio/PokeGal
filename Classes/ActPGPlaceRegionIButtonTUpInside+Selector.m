//
//  ActPGPlaceRegionIButtonTUpInside+Selector.m
//  frdfrd
//
...
#import "ActPGPlaceRegionIButtonTUpInside+Selector.h"
...
@implementation ActPGPlaceRegionIButtonTUpInside(Selector)
...
-(void)setupEventForSchedule{
	...
	event.desc = kRegionDescI;
	event.name = kRegionNameI;
	event.region = kRegionI;
	
	//Edit role, color, image
	event.role = [NSNumber numberWithInt:kAudUDNumPGPlaceRegionI];
	event.color = kRegionColorI;
	event.image = [MLoad getContentWithAttr1:[NSNumber numberWithInt:kModelGeneralScenePGSchedule] 
									   ...
									   Attr3:[NSNumber numberWithInt:kModelG2DKindUiScheRegionI] 
...
-(void)switchViewToPGMainWithTFlipFromL{
	if (![MUi switchViewWithController:@"PGMainSVController" transition:UIViewAnimationTransitionFlipFromLeft])
		NSLog(@"ActPGPlaceRegionIButtonTUpInside - switchViewWithController: failed");	
}
