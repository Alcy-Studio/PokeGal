//
//  ActPGMapNavButtonDTUpInside+Selector.m
//  PocketDraft
//
...
#import "ActPGMapNavButtonDTUpInside+Selector.h"
...
@interface ActPGMapNavButtonDTUpInside()
...
@implementation ActPGMapNavButtonDTUpInside(Selector)
...
-(void)modifyDirectRole{
	...
	[MUi modifyTag:kRegionTagPic role:kAudUDNumPGMapRegionPicD scene:kSceneCodePGMap];
	[MUi modifyTag:kRegionTagTopLvReq role:kAudUDNumPGMapTopLvReqRegionD scene:kSceneCodePGMap];
	[MUi modifyTag:kRegionTagMidLvReq role:kAudUDNumPGMapMidLvReqRegionD scene:kSceneCodePGMap];
	[MUi modifyTag:kRegionTagLowLvReq role:kAudUDNumPGMapLowLvReqRegionD scene:kSceneCodePGMap];
	[MUi modifyTag:kRegionTagDetail role:kAudUDNumPGMapRegionDDetail scene:kSceneCodePGMap];
	
	[MUi modifyTag:kRegionTagGo role:kAudUDNumPGMapGoButtonD scene:kSceneCodePGMap];
...
-(void)modifyInDirectRole{
	...
	NSArray* array2 = [MLoad getRecordsWithAttrName:kGlossaryName attrValue:kRegionNameD Entity:kGlossaryRegion];
	if ([array2 count] != 1) {
		NSLog(@"[ActPGMapNavButton modifyInDirectRole] - Failed");
		return;
	}
...
#pragma mark Test
	...
	if(![MEvent checkEventExist:[NSString stringWithFormat:@"%@ %@",s1,s2] Region:kRegionD]){
		//set TmpAction Go
		[MUi modifyUserDefaultWithKey:kTmpAction value:kAtRegionD];
		//[self playSoundNormal];
		return;
	}
	...
	[MUi modifyUserDefaultWithKey:kTmpAction value:kAtRegionDateD];
	
	//set Role for Dating
	[MUi modifyTag:kRegionTagGo role:kAudUDNumPGMapDateButtonD scene:kSceneCodePGMap];
