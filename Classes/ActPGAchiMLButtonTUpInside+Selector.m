//
//  ActPGAchiMLButtonTUpInside+Selector.m
//  PocketDraft
//
...
#import "ActPGAchiMLButtonTUpInside+Selector.h"
...
@implementation ActPGAchiMLButtonTUpInside(Selector)
...
-(void)setHintWithGiftDescriptionNone{
	NSString* s = [MLoad getContentWithAttr1:[NSNumber numberWithInt:kModelGeneralScenePGAchi] 
									   ...
									   Attr3:[NSNumber numberWithInt:kModelTextOutKindGiftHint4] 
									  Entity:kGlossaryTextOut
										 Key:kGlossaryContent];
	if (!s){
		NSLog(@"ActPGAchiMLButtonTUpInside - setHintWithGiftDescriptionNone");	
		return;
	}
	
	if (![self.resource isMemberOfClass:[PGAchiViewController class]]){
		NSLog(@"ActPGAchiMLButtonTUpInside: c is not PGAchiViewController");
		return;
	}
	id v = [[(PGAchiViewController*)self.resource view] viewWithTag:tag1];
	
	if (![v isMemberOfClass:[UILabel class]]){
		NSLog(@"ActPGAchiMLButtonTUpInside: v is not UILabel");
		return;
	}
...
-(void)setMsgWithGiftDescriptionOn{
	NSString* s = [MLoad getContentWithAttr1:[NSNumber numberWithInt:kModelGeneralScenePGAchi] 
									   Attr2:[NSNumber numberWithInt:kModelTextOutInfoGift] 
									   Attr3:[NSNumber numberWithInt:kModelTextOutKindGiftNote4] 
									  Entity:kGlossaryTextOut
										 Key:kGlossaryContent];
	if (!s){
