//
//  ActPGMapNavButtonBTUpInside+Selector.m
//  PocketDraft
//
...
#import "ActPGMapNavButtonBTUpInside+Selector.h"
...
@interface ActPGMapNavButtonBTUpInside()
...
@implementation ActPGMapNavButtonBTUpInside(Selector)
...
-(void)modifyDirectRole{
	...
	[MUi modifyTag:kRegionTagPic role:kAudUDNumPGMapRegionPicB scene:kSceneCodePGMap];
	[MUi modifyTag:kRegionTagTopLvReq role:kAudUDNumPGMapTopLvReqRegionB scene:kSceneCodePGMap];
	[MUi modifyTag:kRegionTagMidLvReq role:kAudUDNumPGMapMidLvReqRegionB scene:kSceneCodePGMap];
	[MUi modifyTag:kRegionTagLowLvReq role:kAudUDNumPGMapLowLvReqRegionB scene:kSceneCodePGMap];
	[MUi modifyTag:kRegionTagDetail role:kAudUDNumPGMapRegionBDetail scene:kSceneCodePGMap];
	
	[MUi modifyTag:kRegionTagGo role:kAudUDNumPGMapGoButtonB scene:kSceneCodePGMap];
...
-(void)modifyInDirectRole{
	...
	NSArray* array2 = [MLoad getRecordsWithAttrName:kGlossaryName attrValue:kRegionNameB Entity:kGlossaryRegion];
	if ([array2 count] != 1) {
		NSLog(@"[ActPGMapNavButton modifyInDirectRole] - Failed");
		return;
	}
...
#pragma mark Test
	...
	if(![MEvent checkEventExist:[NSString stringWithFormat:@"%@ %@",s1,s2] Region:kRegionB]){
		//set TmpAction Go
		[MUi modifyUserDefaultWithKey:kTmpAction value:kAtRegionB];
		//[self playSoundNormal];
		return;
	}
	...
	[MUi modifyUserDefaultWithKey:kTmpAction value:kAtRegionDateB];
	
	//set Role for Dating
	[MUi modifyTag:kRegionTagGo role:kAudUDNumPGMapDateButtonB scene:kSceneCodePGMap];
