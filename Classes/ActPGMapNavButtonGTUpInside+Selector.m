//
//  ActPGMapNavButtonGTUpInside+Selector.m
//  PocketDraft
//
...
#import "ActPGMapNavButtonGTUpInside+Selector.h"
...
@interface ActPGMapNavButtonGTUpInside()
...
@implementation ActPGMapNavButtonGTUpInside(Selector)


-(void)modifyDirectRole{
	//Change role of Picture, Top Lv, Mid Lv, Low Lv, DetailButton 
	[MUi modifyTag:kRegionTagPic role:kAudUDNumPGMapRegionPicG scene:kSceneCodePGMap];
	[MUi modifyTag:kRegionTagTopLvReq role:kAudUDNumPGMapTopLvReqRegionG scene:kSceneCodePGMap];
	[MUi modifyTag:kRegionTagMidLvReq role:kAudUDNumPGMapMidLvReqRegionG scene:kSceneCodePGMap];
	[MUi modifyTag:kRegionTagLowLvReq role:kAudUDNumPGMapLowLvReqRegionG scene:kSceneCodePGMap];
	[MUi modifyTag:kRegionTagDetail role:kAudUDNumPGMapRegionGDetail scene:kSceneCodePGMap];
	
	[MUi modifyTag:kRegionTagGo role:kAudUDNumPGMapGoButtonG scene:kSceneCodePGMap];
...
-(void)modifyInDirectRole{
	...
	NSArray* array2 = [MLoad getRecordsWithAttrName:kGlossaryName attrValue:kRegionNameG Entity:kGlossaryRegion];
	if ([array2 count] != 1) {
		NSLog(@"[ActPGMapNavButton modifyInDirectRole] - Failed");
		return;
	}
...
#pragma mark Test
	...
	if(![MEvent checkEventExist:[NSString stringWithFormat:@"%@ %@",s1,s2] Region:kRegionG]){
		//set TmpAction Go
		[MUi modifyUserDefaultWithKey:kTmpAction value:kAtRegionG];
		//[self playSoundNormal];
		return;
	}
	...
	[MUi modifyUserDefaultWithKey:kTmpAction value:kAtRegionDateG];
	
	//set Role for Dating
	[MUi modifyTag:kRegionTagGo role:kAudUDNumPGMapDateButtonG scene:kSceneCodePGMap];
