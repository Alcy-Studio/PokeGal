//
//  ActPGMapNavButtonITUpInside+Selector.m
//  PocketDraft
//
...
#import "ActPGMapNavButtonITUpInside+Selector.h"
...
@interface ActPGMapNavButtonITUpInside()
...
@implementation ActPGMapNavButtonITUpInside(Selector)

-(void)modifyDirectRole{
	//Change role of Picture, Top Lv, Mid Lv, Low Lv, DetailButton 
	[MUi modifyTag:kRegionTagPic role:kAudUDNumPGMapRegionPicI scene:kSceneCodePGMap];
	[MUi modifyTag:kRegionTagTopLvReq role:kAudUDNumPGMapTopLvReqRegionI scene:kSceneCodePGMap];
	[MUi modifyTag:kRegionTagMidLvReq role:kAudUDNumPGMapMidLvReqRegionI scene:kSceneCodePGMap];
	[MUi modifyTag:kRegionTagLowLvReq role:kAudUDNumPGMapLowLvReqRegionI scene:kSceneCodePGMap];
	[MUi modifyTag:kRegionTagDetail role:kAudUDNumPGMapRegionIDetail scene:kSceneCodePGMap];
	
	[MUi modifyTag:kRegionTagGo role:kAudUDNumPGMapGoButtonI scene:kSceneCodePGMap];
...
-(void)modifyInDirectRole{
	...
	NSArray* array2 = [MLoad getRecordsWithAttrName:kGlossaryName attrValue:kRegionNameI Entity:kGlossaryRegion];
	if ([array2 count] != 1) {
		NSLog(@"[ActPGMapNavButton modifyInDirectRole] - Failed");
		return;
	}
...
#pragma mark Test
	...
	if(![MEvent checkEventExist:[NSString stringWithFormat:@"%@ %@",s1,s2] Region:kRegionI]){
		//set TmpAction Go
		[MUi modifyUserDefaultWithKey:kTmpAction value:kAtRegionI];
		//[self playSoundNormal];
		return;
	}
	...
	[MUi modifyUserDefaultWithKey:kTmpAction value:kAtRegionDateI];
	
	//set Role for Dating
	[MUi modifyTag:kRegionTagGo role:kAudUDNumPGMapDateButtonI scene:kSceneCodePGMap];
