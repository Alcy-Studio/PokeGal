...
#import "PGDetailViewController+IB.h"
#import "Think.h"
#import "Think+Act.h"
...
@implementation PGDetailViewController(IB)
...
- (IBAction) touchUpInside:(id)sender{
	 ...
	 */
	Think *think = [[Think alloc] initWithScene:kSceneCodePGDetail tag:[sender tag] event: kEventCodeTouchUpInside];
	if (![think act])
...
- (IBAction) PGTouchUpInside:(id)sender{
	...
	if ([sender tag] == kDetailViewTitleButton) {
		action = [[ActPGDetailTitleTUpInside alloc] initWithRole:kDetailViewTitleButton];
	} else {
		return;
	}
