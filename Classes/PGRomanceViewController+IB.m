...
#import "PGRomanceViewController+IB.h"
#import "Think.h"
#import "Think+Act.h"
...
@implementation PGRomanceViewController(IB)
...
- (IBAction) touchUpInside:(id)sender{
	 ...
	 */
	Think *think = [[Think alloc] initWithScene:kSceneCodePGRomance tag:[sender tag] event: kEventCodeTouchUpInside];
	if (![think act])
...
- (IBAction) PGTouchUpInside:(id)sender{
	...
	if ([sender tag] == kRomanceViewTitleButton) {
		action = [[ActPGRomanceTitleTUpInside alloc] initWithRole:kRomanceViewTitleButton];
	} else {
		return;
	}
