//
//  ActPGPlaceRegionCButtonTUpInside+Selector.m
//  frdfrd
//
...
#import "ActPGPlaceRegionCButtonTUpInside+Selector.h"
#import "Event.h"
...
@implementation ActPGPlaceRegionCButtonTUpInside(Selector)
...
-(void)setupEventForSchedule{
	...
	event.desc = kRegionDescC;
	event.name = kRegionNameC;
	event.region = kRegionC;
	
	//Edit role, color, image
	event.role = [NSNumber numberWithInt:kAudUDNumPGPlaceRegionC];
	event.color = kRegionColorC;
	event.image = [MLoad getContentWithAttr1:[NSNumber numberWithInt:kModelGeneralScenePGSchedule] 
									   ...
									   Attr3:[NSNumber numberWithInt:kModelG2DKindUiScheRegionC] 
...
-(void)switchViewToPGMainWithTFlipFromL{
	if (![MUi switchViewWithController:@"PGMainSVController" transition:UIViewAnimationTransitionFlipFromLeft])
		NSLog(@"ActPGPlaceRegionCButtonTUpInside - switchViewWithController: failed");	
}
